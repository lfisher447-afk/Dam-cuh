...
export function capListOverviews<T extends { overview?: string }>(items: T[], maxLength = 240): T[] {
  if (!Array.isArray(items)) return [];
  return items.map((item) => {
    if (!item?.overview || item.overview.length <= maxLength) return item;
    return {
      ...item,
      overview: item.overview.slice(0, maxLength).trim() + "…",
    };
  });
}
...
export function genreNames(ids?: number[]): string[] {
  ...
  return ids.map((id) => GENRE_NAMES[String(id)]).filter(Boolean);
}

export function getMediaTitle(item: any): string {
  if (!item) return "";
  return item.title || item.name || item.original_title || item.original_name || "";
}

export function getMediaReleaseDate(item: any): string {
  if (!item) return "";
  return item.release_date || item.first_air_date || "";
}

export function getReleaseYear(item: any): string {
  const date = getMediaReleaseDate(item);
  return date ? date.split("-")[0] : "";
}

export function mediaDetailHref(type: MediaType | string, id: number | string): string {
  const path = type === "tv" || type === "series" ? "tv-shows" : "movies";
  return `/${path}/${id}`;
}

export function mediaGenreBasePath(type: MediaType | string): string {
  return type === "tv" || type === "series" ? "/tv-shows/genre" : "/movies/genre";
}

export function resolveMediaType(item: any): MediaType {
  if (item?.media_type === "tv" || item?.first_air_date || item?.name) return "tv";
  return "movie";
}

export function toListEntries(items: any[], defaultType?: MediaType) {
  if (!Array.isArray(items)) return [];
  return items
    ...
    path: mediaDetailHref(item.media_type || defaultType || "movie", item.id),
  }));
}
...
export function getMediaHeroImageUrl(
  ...
  if (!backdropOrItem && !posterPath) return "";
  if (backdropOrItem && typeof backdropOrItem === "object") {
    const path = backdropOrItem.backdrop_path || backdropOrItem.poster_path;
    if (!path) return "";
    if (typeof path === "string" && path.startsWith("http")) return path;
    return `https://image.tmdb.org/t/p/w1280${path}`;
  }
  const path = (typeof backdropOrItem === "string" ? backdropOrItem : null) || posterPath;
  if (!path) return "";
  if (typeof path === "string" && path.startsWith("http")) return path;
  return `https://image.tmdb.org/t/p/w1280${path}`;
}
