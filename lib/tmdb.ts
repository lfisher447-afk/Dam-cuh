...
export function imageUrl(
  ...
  size: "w92" | "w185" | "w300" | "w342" | "w500" | "w780" | "w1280" | "original" = "w500",
): string {
...
export function tvToMedia(item: any) {
  if (!item) return item;
  return {
    ...item,
    id: item.id,
...
export async function getTrendingTV() {
  const data = await tmdbFetch("/trending/tv/day").catch(() => ({ results: [] }));
  return data?.results ?? [];
}

export async function getPopularTV() {
  const data = await tmdbFetch("/tv/popular").catch(() => ({ results: [] }));
  return data?.results ?? [];
}

export async function getTopRatedTV() {
  const data = await tmdbFetch("/tv/top_rated").catch(() => ({ results: [] }));
  return data?.results ?? [];
}

export async function getAiringTodayTV() {
  const data = await tmdbFetch("/tv/airing_today").catch(() => ({ results: [] }));
  return data?.results ?? [];
}

export async function getOnTheAirTV() {
  const data = await tmdbFetch("/tv/on_the_air").catch(() => ({ results: [] }));
  return data?.results ?? [];
}
...
export function logoUrl(path?: string | null, size: string = "w500"): string {
  if (!path) return "";
  if (path.startsWith("http")) return path;
...
export function pickMovieLogo(images: any): any {
  const logos = images?.logos ?? images ?? [];
  if (!Array.isArray(logos) || !logos.length) return null;
  const enLogo = logos.find((l: any) => l.iso_639_1 === "en");
  return enLogo || logos[0] || null;
}

export function pickTVLogo(images: any): any {
  return pickMovieLogo(images);
}

export function movieToMedia(item: any) {
  if (!item) return item;
  return {
    ...item,
    id: item.id,
...
export async function getSeasonEpisodes(tvId: string | number, seasonNumber: string | number) {
  return tmdbFetch(`/tv/${tvId}/season/${seasonNumber}`);
}

export async function getNowPlaying() {
  const data = await tmdbFetch("/movie/now_playing").catch(() => ({ results: [] }));
  return data?.results ?? [];
}

export async function getPopular() {
  const data = await tmdbFetch("/movie/popular").catch(() => ({ results: [] }));
  return data?.results ?? [];
}

export async function getTopRated() {
  const data = await tmdbFetch("/movie/top_rated").catch(() => ({ results: [] }));
  return data?.results ?? [];
}

export async function getTrending() {
  const data = await tmdbFetch("/trending/movie/day").catch(() => ({ results: [] }));
  return data?.results ?? [];
}

export async function getUpcoming() {
  const data = await tmdbFetch("/movie/upcoming").catch(() => ({ results: [] }));
  return data?.results ?? [];
}
