...
import { getMediaReleaseDate, getMediaTitle, getReleaseYear } from '@/lib/media'
...
export function buildMediaEventBase(media: MediaEventMedia, type: MediaKind) {
  const parsedReleaseYear = Number(getReleaseYear(getMediaReleaseDate(media)))
  return {
    media_id: media.id,
    media_type: type,
    title: getMediaTitle(media),
    vote_average: media.vote_average,
    release_year: Number.isFinite(parsedReleaseYear)
      ? parsedReleaseYear
      : undefined,
    genres: media.genres
