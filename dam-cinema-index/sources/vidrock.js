...
function getStreamProxyHeaders(streamUrl) {
    try {
        ...
        if (hostname === 'play.xpass.top' || hostname.endsWith('.xpass.top')) return { 'Referer': 'https://play.xpass.top/', 'Origin': 'https://play.xpass.top' };
    } catch { }
    return { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/134.0.6884.98 Safari/537.36', 'Accept': '*/*', 'Accept-Language': 'en-US,en;q=0.9', 'Referer': BASE_URL, 'Origin': BASE_URL.replace(/\/$/, '') };
}
...
export async function getStream({ id, s, e }) {
    try {
        ...
        await Promise.all(Object.values(data).map(async (stream) => {
            ...
            if (streamUrl.startsWith(PROXY_PREFIX)) streamUrl = decodeURIComponent(streamUrl.slice(PROXY_PREFIX.length).replace(/^\//, ''));
            const fetchedData = await fetchPage(streamUrl).catch(() => null);
            if (Array.isArray(fetchedData)) {
                for (const obj of fetchedData) {
                    if (!obj?.url) continue;
                    let finalUrl = obj.url.startsWith(PROXY_PREFIX) ? decodeURIComponent(obj.url.slice(PROXY_PREFIX.length).replace(/^\//, '')) : obj.url;
