...
export function tasteQuery(picks: TastePick[]) {
  ...
  for (const pick of picks) {
    if (pick.type === "movie") movieCount++;
    else if (pick.type === "tv" || pick.type === "series") tvCount++;
    for (const g of pick.genre_ids ?? []) {
      genreCounts[g] = (genreCounts[g] ?? 0) + 1;
    }
  }

  const topGenre = Object.entries(genreCounts).sort((a, b) => b[1] - a[1])[0]?.[0];
  const mediaType: MediaType = tvCount > movieCount ? "tv" : "movie";

  const params: Record<string, string> = {
    sort_by: "popularity.desc",
  };
  if (topGenre) {
    params.with_genres = topGenre;
  }
  return { mediaType, params };
}

export function tastePrompt(count: number): string {
  if (count < TASTE_MIN) {
    return `Pick ${TASTE_MIN - count} more to unlock suggestions`;
  }
  return `Ready! Selected ${count} titles`;
}
